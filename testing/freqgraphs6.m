...
start_index_1 = 500; % Starting index for the first range
...
non_zero_indices_1 = 1:length(f1); % Exclude the first index (0 frequency)
...
non_zero_indices_2 = 1:length(f2); % Exclude the first index (0 frequency)
...
title('FFT of Temperature Data (Failing, Excluding 0 Hz)');
xlabel('Frequency (Hz)');
ylabel('Magnitude');
grid on;

% Adjust the layout
sgtitle('Temperature Data and FFT Analysis (Failing)');
