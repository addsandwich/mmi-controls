...
subplot(4,1,1);  % Three rows, one column, first plot
...
subplot(4,1,2);  % Three rows, one column, second plot
...
subplot(4,1,3);  % Three rows, one column, third plot
...
subplot(4,1,4);  % Three rows, one column, third plot
surf(t_utc, f, p_db, 'EdgeColor', 'none');
axis tight;
view(0, 90);
xlabel('Time (UTC)');
