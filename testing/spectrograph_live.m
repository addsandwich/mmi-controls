...
for i = 1:length(new_time_utc)
    % Update the interpolated signal plot
    set(h1, 'XData', new_time_utc(1:i), 'YData', interpolated_signal(1:i));
    
    % Update the average power plot
    if i <= length(avg_power)
        set(h2, 'XData', avg_power_time(1:i), 'YData', avg_power(1:i));
    end
    
    % Update the spectrogram plot
    if i <= length(t)
        set(h3, 'XData', t_utc(1:i), 'CData', p_db(:, 1:i));
    end
